...
export function parseNonNpcRefReclassificationArgs(argv = process.argv.slice(2)) {
  ...
  if (apply && !confirmLocalCompat) {
    throw new Error('non-NPC ref reclassification apply requires --confirm-local-compat=true');
  }
  if (apply && !allowBulk) {
    throw new Error('non-NPC ref reclassification apply requires --allow-bulk=true');
  }
  return {
    outputPath: options.output ?? null,
    backupDir: options['backup-dir'] ?? options.backupDir ?? path.join(process.cwd(), 'data', 'backups', 'item-source-non-npc-ref-reclassification'),
...
export function classifyNonNpcSource(name, sourceType) {
  const text = normalizeDisplayText(name) ?? '';
  const normalizedSourceType = normalizeText(sourceType);
  if (normalizedSourceType === 'drop' && /\b(?:palm\s+)?tree$/i.test(text)) {
    return { sourceRefType: 'world', reason: 'tree_source' };
  }
  if (normalizedSourceType === 'drop' && /^Giant Glowing Mushroom$/i.test(text)) {
    return { sourceRefType: 'world', reason: 'environment_source' };
  }
  if (normalizedSourceType === 'drop' && /(?:heart|orb)$/i.test(text)) {
    return { sourceRefType: 'world', reason: 'heart_or_orb_source' };
  }
  if (normalizedSourceType === 'drop' && /^(Celestial Pillars|Shadow Hammer)$/i.test(text)) {
    return { sourceRefType: 'world', reason: 'world_object_source' };
  }
  if (/^seed:/i.test(text)) {
    return { sourceRefType: 'world', reason: 'world_seed_condition_source' };
  }
  if (/^Mechdusa$/i.test(text)) {
    return { sourceRefType: 'boss_group', reason: 'boss_lane_reference_source' };
  }
  if (/^(?:the|her for)$/i.test(text)) {
    return { sourceRefType: 'unknown', reason: 'malformed_source_name' };
  }
  return null;
}

export function buildNonNpcRefReclassificationPlan({ sourceRows = [] } = {}) {
  const updates = [];
  const blocked = [];
  for (const row of Array.isArray(sourceRows) ? sourceRows : []) {
    ...
    const classification = classifyNonNpcSource(sourceRefName, row.source_type);
    if (!classification) {
      blocked.push(blockedRow(row, 'not_reviewed_non_npc_source'));
      continue;
    }
    updates.push({
      id: Number(row.id),
...
export async function runNonNpcRefReclassification(options = {}, dependencies = {}) {
  const now = dependencies.now instanceof Date ? dependencies.now : new Date();
  const batchId = options.batchId ?? `item-source-non-npc-ref-reclassification-${now.toISOString().replace(/[:.]/g, '-')}`;
  ...
  if (options.apply && connectionConfig.database !== 'terria_v1_local') {
    throw new Error(`Refusing non-NPC ref reclassification apply to non-local database: ${connectionConfig.database}`);
  }
  ...
  try {
    ...
    plan = buildNonNpcRefReclassificationPlan({ sourceRows });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runNonNpcRefReclassification(parseNonNpcRefReclassificationArgs())
    .then((report) => {
