...
export function buildDomainAcceptanceFreshnessAudit({
  ...
  const selectedDomainIds = normalizeDomainIds(domainIds);
  const now = parseDate(generatedAt) ?? new Date();
  const entries = selectedDomainIds
    ? manifest.filter((entry) => selectedDomainIds.has(entry.domainId))
    : manifest;
  const panels = entries.map((entry) => buildPanelFreshness({ entry, repoRoot: root, now }));
...
export function classifyDomainAcceptanceCommandRisk(command = '') {
...
function buildPanelFreshness({ entry, repoRoot, now }) {
  const commandRisk = classifyDomainAcceptanceCommandRisk(entry.generatorCommand);
  const base = {
  ...
  if (!reportTime.generatedAt) {
    return {
      ...
      latestReportPath: latestReport.relativePath,
      freshnessStatus: 'unknown',
      freshnessReason: 'Domain acceptance report time is unavailable.',
    };
  }
...
function summarizePanels(panels) {
  return {
    ...
    domainCount: new Set(panels.map((panel) => panel.domainId)).size,
...
function summarizeGate(panels) {
  const blockingReasons = panels
    ...
    .map((panel) => `${panel.domainId}/${panel.panelId} generator command is unsafe`);
  const warningReasons = panels
    .filter((panel) => ['missing', 'stale', 'unknown'].includes(panel.freshnessStatus))
    .map((panel) => `${panel.domainId}/${panel.panelId} evidence is ${panel.freshnessStatus}`);
  return {
...
function normalizeDomainIds(domainIds) {
  ...
  const values = Array.isArray(domainIds)
    ? domainIds
    : String(domainIds).split(',');
