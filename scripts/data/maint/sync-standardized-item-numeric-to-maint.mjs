...
export function buildMaintItemNumericOverrides({
  standardizedItems = [],
} = {}) {
  return standardizedItems
    .map((item) => ({
      item,
      row: {
        itemInternalName: String(item?.internalName ?? '').trim() || null,
        damageValue: toNullableNumber(item?.stats?.damage),
        defenseValue: toNullableNumber(item?.stats?.defense),
        knockbackValue: toNullableNumber(item?.stats?.knockback),
        useTime: toNullableNumber(item?.stats?.useTime),
        buyValue: toNullableNumber(item?.economy?.buy),
        sellValue: toNullableNumber(item?.economy?.sell),
      }
    }))
    .filter(({ row }) => row.itemInternalName && hasNumericCoverage(row))
    .map(({ item, row }) => ({
      recordKey: buildRecordKey(`maint_item_numeric_overrides:${row.itemInternalName}`),
      ...row,
...
async function upsertRows(connection, rows) {
  for (const row of rows) {
    ...
    await connection.execute(
      `INSERT INTO \`maint_item_numeric_overrides\` (${columns.map((column) => `\`${column}\``).join(', ')}) VALUES (${placeholders}) ON DUPLICATE KEY UPDATE ${updates}`,
...
async function run() {
  ...
  const output = path.resolve(process.cwd(), args.output ?? path.join('reports', 'relation', `item-numeric-override-sync-${new Date().toISOString().slice(0, 10)}.json`));
  ...
  try {
    ...
    const rows = buildMaintItemNumericOverrides({ standardizedItems });
