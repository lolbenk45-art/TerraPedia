...
async function ensureLookupTables(conn) {
  ...
  await conn.query(`
    ...
    VALUES
      (1, 'pre_hardmode', '前期', 'Pre-Hardmode', 1, 1, 0),
...
async function loadIteminfoModuleData() {
  const helperPath = pathToFileURL('G:/ClaudeCode/terraPedia/scripts/data/lib/wiki-item-utils.mjs').href;
  const helper = await import(helperPath);
  const raw = JSON.parse(fs.readFileSync('G:/ClaudeCode/terraPedia/data/raw/wiki/module__iteminfo__data.latest.json', 'utf8'));
