...
usage() {
  cat <<'USAGE'
Usage: scripts/ops/run-backend-refresh-daemon-host.sh [options]

Runs the TerraPedia backend refresh daemon in the foreground.
...
Options:
  ...
  --once                     Run one refresh cycle instead of the long-running daemon
