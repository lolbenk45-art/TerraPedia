export type RecipeGraphStation = { stationType?: string | null, stationItemId?: string | number | null, stationName?: string | null, stationNameZh?: string | null, name?: string | null, displayName?: string | null, stationInternalName?: string | null, stationImage?: string | null, itemImage?: string | null, itemImageUrl?: string | null, image?: string | null }
export type RecipeGraphMember = { itemId?: string | number | null, internalName?: string | null, name?: string | null, nameZh?: string | null, image?: string | null, imageUrl?: string | null }
export type RecipeGraphNode = {
  ...
  children?: RecipeGraphNode[] | null
  stations?: RecipeGraphStation[] | null
  groupMembers?: RecipeGraphMember[] | null
  recipeOptions?: RecipeGraphNode[][] | null
}

type LayoutNode = { id: string, node: RecipeGraphNode, depth: number, isRoot: boolean, x: number, y: number, width: number, height: number, anchorX: number, subtreeWidth: number, children: LayoutNode[] }
...
const childrenOf = (node: RecipeGraphNode) => Array.isArray(node.children) ? node.children : []
const nameOf = (node: RecipeGraphNode) => firstText(node.displayName, node.itemNameZh, node.itemName, node.name, node.itemInternalName, '材料')
const quantityOf = (node: RecipeGraphNode, isRoot = false) => {
...
const keyOf = (node: RecipeGraphNode) => firstText(node.itemId, node.id, node.itemInternalName, node.itemName, node.displayName, node.name)
const sameItem = (left: RecipeGraphNode, right: RecipeGraphNode) => {
...
const imageValueOf = (node: RecipeGraphNode) => firstText(node.itemImage, node.itemImageUrl, node.image, node.previewImage, node.groupMembers?.[0]?.image, node.groupMembers?.[0]?.imageUrl)
const groupMembersOf = (node: RecipeGraphNode) => Array.isArray(node.groupMembers) ? node.groupMembers : []
const groupMemberName = (member: RecipeGraphMember) => firstText(member.nameZh, member.name, member.internalName, '可替代材料')
const groupMemberImage = (member: RecipeGraphMember) => firstText(member.image, member.imageUrl)
const nodeGroupMember = (node: RecipeGraphNode): RecipeGraphMember => ({
  itemId: node.itemId,
...
const stationsOf = (node: RecipeGraphNode): RecipeGraphStation[] => {
...
const stationName = (station: RecipeGraphStation | null | undefined) => firstText(station?.displayName, station?.stationNameZh, station?.stationName, station?.name, station?.stationInternalName)
const stationImage = (station: RecipeGraphStation | null | undefined) => firstText(station?.stationImage, station?.itemImage, station?.itemImageUrl, station?.image)
const optionGroupsOf = (node: RecipeGraphNode) => node.nodeType === 'recipe_options' && Array.isArray(node.recipeOptions)
  ? node.recipeOptions
  : []

const mergeAlternativeChildren = (options: RecipeGraphNode[]): RecipeGraphNode[] => {
  ...
  const shared = new Map<string, RecipeGraphNode>()
  const alternatives: RecipeGraphNode[] = []
...
const mergeSameItemSiblings = (nodes: RecipeGraphNode[]) => {
  const groups = new Map<string, RecipeGraphNode[]>()
  const order: string[] = []
...
const graphChildren = (node: RecipeGraphNode, depth: number, maxDepth: number): RecipeGraphNode[] => {
...
const cardWidth = (node: RecipeGraphNode) => optionGroupsOf(node).length ? OPTION_SOURCE_WIDTH : CARD_WIDTH
const cardHeight = (node: RecipeGraphNode) => {
...
const buildLayout = (node: RecipeGraphNode, depth: number, maxDepth: number, indexPath = '0', isRoot = false): LayoutNode => {
  let normalized = node
...
const layoutForest = (roots: RecipeGraphNode[], maxDepth: number): Layout => {
...
const relationLabel = (node: RecipeGraphNode, depth: number, isRoot: boolean) => optionGroupsOf(node).length ? '' : isRoot ? 'ROOT' : groupMembersOf(node).length > 1 ? `任选 ${groupMembersOf(node).length}` : graphChildren(node, depth, depth + 1).length ? '子配方' : '材料'
const detailRows = (node: RecipeGraphNode, depth: number, relation: string, quantity: string) => {
...
export const renderRecipeHierarchyGraph = (options: RecipeHierarchyGraphRendererOptions) => {
  ...
  const runtimeThemeClass = options.popoverThemeClass.replace('tp-article-runtime-popover', 'tp-article-runtime')
  const graph = document.createElement('div'); graph.className = `tp-article-runtime ${runtimeThemeClass} recipe-hierarchy-tree--article-embed recipe-hierarchy-tree recipe-overview-tree`; graph.setAttribute('data-crafting-role', 'recipe-hierarchy-tree'); graph.dataset.recipeHierarchyRenderer = 'shared'
