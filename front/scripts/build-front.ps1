...
$logFile = Join-Path $logDir "front-build.log"
$stderrFile = Join-Path $logDir "front-build.stderr.log"
...
Write-Host "Frontend build log saved to $logFile"
...
if ($exitCode -ne 0) {
  throw "Frontend build failed with exit code $exitCode"
}
