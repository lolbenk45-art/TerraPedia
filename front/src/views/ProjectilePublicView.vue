<template>
  <div class="entity-list-view public-workbench page-wrap">
    <section class="public-page-hero entity-list-view__hero">
      <div class="public-page-hero__layout">
        <div class="public-page-hero__copy">
          ...
          <h1 class="section-title section-title--hero">Projectile Archive</h1>
          <p class="section-copy section-copy--wide">
            Browse public projectile records with managed sprite fallback, combat flags, and quick damage snapshots.
          </p>
        </div>
        <div class="public-page-hero__meta entity-list-view__hero-meta">
          ...
          <article class="public-hero-stat-card"><span class="public-hero-stat-card__label">Focus</span><strong class="public-hero-stat-card__value">{{ activeSearch ? 'Filtered' : 'All public projectiles' }}</strong></article>
    ...
    <section class="public-summary-strip entity-list-view__filters">
      <form class="entity-list-view__toolbar" @submit.prevent="applyFilters">
        <label class="entity-list-view__field">
          <span>Search Projectiles</span>
          <input v-model.trim="searchInput" type="search" class="entity-list-view__input" placeholder="Search by projectile name or internal name" />
        </label>
    ...
    <section v-if="isLoading" class="public-section-frame entity-list-view__state"><div class="entity-list-view__spinner" aria-hidden="true"></div><p>Loading projectiles...</p></section>
    <section v-else-if="error" class="public-section-frame entity-list-view__state entity-list-view__state--error"><strong>Could not load projectiles</strong><p>{{ error }}</p><button type="button" class="btn btn-brand" @click="reloadCurrentRoute">Retry</button></section>

    <template v-else>
      <section class="public-section-frame entity-list-view__summary">
        <div><p class="entity-list-view__caption">Current slice</p><h2>{{ activeSearch ? 'Search results' : 'All public projectiles' }}</h2></div>
        <div class="entity-list-view__meta"><span>{{ pagination.total }} results</span><span>Page {{ pagination.page }} / {{ totalPages }}</span></div>
      </section>

      <section v-if="projectiles.length === 0" class="public-section-frame entity-list-view__state"><strong>No projectiles found</strong><p>Try a broader search term.</p></section>

      <section v-else class="entity-list-view__grid">
        <article v-for="projectile in projectiles" :key="projectile.id" class="public-section-frame entity-card">
          <div class="entity-card__portrait">
            <img v-if="resolveImage(projectile.imageUrl)" :src="resolveImage(projectile.imageUrl)" :alt="displayName(projectile)" loading="lazy" />
            <div v-else class="entity-card__fallback">
              <span class="entity-card__fallback-mark" aria-hidden="true">{{ fallbackMark(projectile.internalName) }}</span>
              <span>No managed sprite</span>
            </div>
          </div>

          <div class="entity-card__body">
            <div class="entity-card__badges">
              <span class="entity-card__badge entity-card__badge--accent">AI {{ projectile.aiStyle ?? '--' }}</span>
              <span class="entity-card__badge">{{ projectile.hostile ? 'Hostile' : projectile.friendly ? 'Friendly' : 'Neutral' }}</span>
            </div>

            <div class="entity-card__heading">
              <h3>{{ displayName(projectile) }}</h3>
              <p v-if="projectile.internalName" class="entity-card__secondary">{{ projectile.internalName }}</p>
            </div>

            <p class="entity-card__summary">Damage {{ projectile.damage ?? '--' }} · Knockback {{ projectile.knockBack ?? '--' }}</p>

            <dl class="entity-card__facts">
              <div><dt>Friendly</dt><dd>{{ projectile.friendly === null ? '--' : projectile.friendly ? 'Yes' : 'No' }}</dd></div>
              <div><dt>Hostile</dt><dd>{{ projectile.hostile === null ? '--' : projectile.hostile ? 'Yes' : 'No' }}</dd></div>
            </dl>
          </div>
        </article>
      </section>
...
import { fetchProjectiles } from '@/api'
import type { Pagination, ProjectileListItem } from '@/types'

const route = useRoute()
const router = useRouter()
const pageSize = 12
const projectiles = ref<ProjectileListItem[]>([])
...
const displayName = (projectile: ProjectileListItem) => projectile.nameZh?.trim() || projectile.name?.trim() || projectile.internalName?.trim() || 'Unknown Projectile'
...
const fallbackMark = (value?: string | null) => value?.trim()?.slice(0, 2).toUpperCase() || 'PJ'

const loadProjectiles = async () => {
  isLoading.value = true
  error.value = ''
  try {
    const response = await fetchProjectiles(pagination.value.page, pageSize, activeSearch.value || undefined)
    if (!response.success) throw new Error(response.message || 'Failed to fetch projectile list')
    projectiles.value = response.data || []
    pagination.value = {
      total: response.pagination?.total ?? projectiles.value.length,
    ...
    error.value = cause instanceof Error ? cause.message : 'Failed to fetch projectile list'
  } finally {
...
const pushRouteState = async (page: number) => {
  ...
  await router.push({ path: '/projectiles', query })
  await loadProjectiles()
}
