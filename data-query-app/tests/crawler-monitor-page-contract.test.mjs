...
test('domain cards and table show current status beside the latest terminal result', () => {
  ...
  assert.match(page, /latestSuccessfulV2AttemptsByDomain/)
  assert.match(page, /localDataUpdateLabel/)
  assert.match(triageBoard, /最近数据/)
  assert.match(triageBoard, /row\.localDataSummary/)
  assert.match(triageBoard, /上游检查/)
  assert.match(triageBoard, /row\.upstreamCheckSummary/)
  assert.match(triageBoard, /<th>最近数据<\/th>/)
  assert.match(triageBoard, /<th>上游检查<\/th>/)
