...
const crawlerTriageBoard = read('components/crawler-monitor/CrawlerTriageBoard.vue')
...
test('admin sidebar sections can collapse without changing the current link style', () => {
  ...
  assert.match(layout, /<div[\s\S]*v-show="!isMenuSectionCollapsed\(section\.label\)"[\s\S]*class="sidebar__section-items"/)
  assert.match(layout, /const uiPreferences = useUiPreferencesStore\(\)/)
  assert.match(layout, /uiPreferences\.applySectionDefaults\(defaultCollapsedSectionLabels\)/)
...
test('crawler monitor primary operation labels are Chinese-first', () => {
  ...
  assert.match(crawlerTriageBoard, /暂无基础域/)
  assert.match(crawlerTriageBoard, /没有匹配的基础域/)
