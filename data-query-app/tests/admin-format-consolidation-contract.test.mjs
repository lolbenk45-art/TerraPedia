...
test('NPC price chips are global while retaining each surface sizing', () => {
  ...
  assert.match(mainCss, /\.town-npc-index \.coin-chip\s*\{[\s\S]*min-height:\s*26px;/)
  assert.match(mainCss, /\.workbench-shell \.coin-chip\s*\{[\s\S]*min-height:\s*28px;/)
  assert.match(mainCss, /\.town-npc-index \.coin-chip__icon\s*\{[\s\S]*width:\s*15px;/)
  assert.match(mainCss, /\.workbench-shell \.coin-chip__icon\s*\{[\s\S]*width:\s*16px;/)
  assert.match(mainCss, /\.workbench-shell \.coin-chip--soft\s*\{[\s\S]*border-color:\s*rgba\(20, 184, 166, 0\.22\);/)
  assert.match(mainCss, /\.workbench-shell \.price-pill--soft\s*\{[\s\S]*background:\s*linear-gradient\(135deg, #0f766e, #14b8a6\);/)
})
