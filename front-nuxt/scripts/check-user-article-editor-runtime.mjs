...
if (!recipeHierarchyGraphRendererSource.includes('recipe-hierarchy-option-row')) throw new Error('shared recipe tree graph renderer must reuse crafting page compact recipe option rows')
if (!recipeHierarchyGraphRendererSource.includes('recipe-hierarchy-popover') || !recipeHierarchyGraphRendererSource.includes('positionPopover')) throw new Error('shared recipe tree graph renderer must expose crafting-style hover basic info popovers')
if (!recipeHierarchyGraphRendererSource.includes('enableInteractions') || !recipeHierarchyGraphRendererSource.includes("addEventListener('wheel'")) throw new Error('shared recipe tree graph renderer must expose invisible wheel zoom and drag pan controls')
