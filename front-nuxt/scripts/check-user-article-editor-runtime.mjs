...
for (const [pageName, pageSource] of [['new', newArticlePageSource], ['edit', editArticlePageSource]]) {
  ...
  if (!/--article-reference-panel-top:\s*clamp\(152px,\s*18dvh,\s*188px\);/.test(pageSource)) throw new Error(`${pageName} article page reference panel must clear the writing toolbar on desktop`)
  ...
  if (!/@media\s*\(max-width:\s*1180px\)\s*\{[\s\S]*\.article-focus-shell--writing\s+\.article-focus-status\s*\{[\s\S]*position:\s*fixed;[\s\S]*--article-reference-panel-top:\s*150px;[\s\S]*top:\s*var\(--article-reference-panel-top\);[\s\S]*width:\s*min\(320px,\s*calc\(100vw - 24px\)\);/.test(pageSource)) throw new Error(`${pageName} article page must keep the reference panel fixed below the writing toolbar on narrow screens`)
}
