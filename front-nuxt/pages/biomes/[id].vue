      ...
      <template v-else>
        <section class="category-detail-grid">
          <article class="support-panel category-branch active">
            <b>资源</b>
            <span>{{ biomeResources.length }} 项资源来自公开接口。</span>
        ...
        <section class="search-suggestion-band support-panel">
          <a v-for="resource in biomeResources" :key="displayText(resource.id, resource.itemId, resource.resourceNameRaw, 'resource')" :href="resource.itemId ? `/items/${resource.itemId}` : '/items'">
            <CommonPreviewImage
              ...
              :alt="displayText(resource.resourceNameRaw, resource.itemName, resource.itemInternalName, '资源')"
              :fallback="firstGlyph(displayText(resource.resourceNameRaw, resource.itemName, resource.itemInternalName, '?'))"
              width="40"
              height="40"
            />
            <b>{{ displayText(resource.resourceNameRaw, resource.itemName, resource.itemInternalName, '未命名资源') }}</b>
            <span>{{ displayText(resource.resourceType, resource.notes, '类型未标注') }}</span>
          </a>
          <div v-if="!biomeResources.length">
            <b>暂无资源</b>
            <span>公开接口没有返回资源记录。</span>
          </div>
        </section>
