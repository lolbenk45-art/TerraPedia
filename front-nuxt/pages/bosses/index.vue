    ...
    <div class="page-head entity-head">
      <div class="page-head-inner">
        <div>
          ...
          <h1>Boss 路线</h1>
          <p>Boss 列表来自公共接口，按推进顺序展示触发方式、部件数量和掉落覆盖情况。</p>
        </div>
    ...
    <main class="boss-page-shell" :aria-busy="bossVisualLoading">
      <section class="boss-command">
        <div>
          ...
          <h2>推进节点与掉落入口</h2>
          <p>加载期间只显示骨架；接口不可用时保持空状态，避免静态样例误导。</p>
        </div>
        ...
        <div class="boss-command-stats">
          <div><b>{{ bossStatusLabel }}</b><span>接口状态</span></div>
      ...
      <section v-if="!bossVisualLoading && !bossDisplayItems.length" class="search-suggestion-band support-panel">
        <div>
          ...
          <span>{{ bossApiUnavailable ? '当前公共接口暂不可用，页面不会展示静态样例。' : '调整搜索词或清空搜索。' }}</span>
        </div>
