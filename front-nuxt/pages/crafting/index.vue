...
const nodeTitle = (node: PublicItemRecipeTreeNode) => displayText(node.displayName, node.itemNameZh, node.itemName, node.itemInternalName, '配方节点')
    ...
    <main class="crafting-layout" :aria-busy="recipeVisualLoading">
      <section class="crafting-command support-panel">
        <div>
          ...
          <p>{{ hasSelectedTarget ? '当前页面只显示 API 返回的配方树。' : '输入物品名称，选择建议后加载 recipe-tree 接口。' }}</p>
          <div class="tag-row">
      ...
      <section v-if="recipeSearchQuery || itemSearchPending || showSearchUnavailable" class="search-suggestion-band support-panel">
        ...
        <div v-else>
          ...
          <span>{{ showSearchUnavailable ? '当前物品接口不可用，已隐藏本地样例建议。' : '调整搜索词后重试。' }}</span>
