...
const articleDraftStorageKey = 'terrapedia:article-draft:new'
...
const readStoredArticleDraft = (): StoredArticleDraft | null => {
  ...
  try {
    const raw = window.localStorage.getItem(articleDraftStorageKey)
    if (!raw) return null
...
onMounted(() => {
  ...
  restorableDraft.value = readStoredArticleDraft()
})
      ...
      <section class="article-writing-surface">
        <div v-if="restorableDraft" class="user-form-status article-draft-restore" role="status">
          ...
          <span class="article-draft-restore__actions">
            <button class="secondary-button" type="button" @click="restoreArticleDraft">恢复草稿</button>
