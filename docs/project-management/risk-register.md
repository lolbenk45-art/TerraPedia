...
| R-2026-05-23-04 | Medium | Evidence retention | Some required source and audit artifacts are ignored by `.gitignore`, so a gate can pass locally while raw producer evidence is not recoverable on another machine | Record stdout summaries in tracked `docs/audits/**`; only track ignored generated artifacts after an explicit evidence-retention decision | Open |
