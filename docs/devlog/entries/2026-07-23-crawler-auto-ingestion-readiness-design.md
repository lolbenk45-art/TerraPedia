...
- Task 4 repair round 5: split V2 child persistence into immutable reservation and immutable attached-identity facts; added no-database migration/FK/CAS contracts; reloads current policy state before evaluation; validates every run-policy row's policy-set hash; requires an active Spring transaction and proves rollback propagation. Execution uses only an injected trusted `ApplyProtocolExecutor` whose default Bean fails closed. All five repair items verified: CrawlerAutomationPolicyServiceImplTest 26/26, frozen-apply-bundle.test 8/8, policy-set-hash.test 4/4, crawler-automation-migration-contract.test 3/3. `git diff --check` clean. Task 4 is complete; Task 5 begins next.
