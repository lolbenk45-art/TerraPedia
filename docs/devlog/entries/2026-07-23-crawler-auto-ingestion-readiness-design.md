...
- Task 7 admin workflow: `AdminCrawlerAutomationController` exposes overview/runs/approvals/profile under `/admin/crawler-automation`; read-only T2 profile returns 403 on approval submission; `CrawlerAutomationServiceImpl` delegates approval to `policyService.consumeApproval` and blocks mutations when `readOnlyProfile=true`; `CrawlerAutomationRunMapper` adds `findActiveDomainSummaries` and `findRecentDecisionContexts`; DTOs: OverviewDTO, RunDTO, ApprovalRequestDTO; `crawler-automation.contract.test.mjs` 9/9; `AdminCrawlerAutomationControllerTest` 12/12; total Java 38/38. `git diff --check` clean. No database, Redis, crawler, import, apply, or rollback command was run.
