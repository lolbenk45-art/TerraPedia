...
- Task 6 operation registry: `fixtures/crawler-automation-capabilities.json` encodes all 19 registered operations with exact actionId, mode, default L0+DISABLED state, owned tables (for write ops), read dependencies, snapshot/rollback mode, and symmetric preview/apply pairing; `capability-manifest.test.mjs` validates 12 contracts including exclusive npc_loot/boss_loot partitions, symmetric pairs, and required fields. Tests: 12/12. `git diff --check` clean. No database, Redis, crawler, import, apply, or rollback command was run.
