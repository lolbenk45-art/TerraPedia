...
- Shimmer retry-02 越过 canonical 路径校验后在 DML 前因 frozen preview drift 回滚，run/apply 行仍为 0、四表计数不变。旧 proposal 的 snapshot 视图为 14→21，而 2026-08-13 导入后的当前基线已为 21，generation/contract 未漂移但 before-state 已过期。修复后的 reuse 路径保留旧 proposal 作为 contract 身份证据，同时对同一 verified generation 和当前 DB 在内存中重算 read-only proposal，不覆盖旧 canonical proposal；RED/GREEN 综合回归 58/58 通过。retry-02 identity 已单次消费，等待新代码 checkpoint、重建 Shimmer preview 与 retry-03 授权。
- Read-only Shimmer refresh 的最终边界已收紧为：generation、manifest、data bundle、provider scope 必须不变；preview/target 身份允许因当前 DB 基线刷新而重算。对应回归 58/58 通过，仍不覆盖旧 canonical proposal。
- retry-03 在 DML 前仍发现预览/apply hash 差异；逐字段重算确认唯一根因是 proposal read-only 连接启用 `dateStrings: true`，正式 runner 连接未启用，导致 snapshot DATETIME 的 JS Date/string 表示不同。统一 supplementary runner 连接选项并新增确定性合同测试，回归 59/59 通过；retry-03 identity 已消费且无数据库写入，等待 retry-04。
- See git for code-level diff details.
