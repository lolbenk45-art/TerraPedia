...
- [ ] **Step 1: 写失败合同**
  ...
  把三个新组件加入 `contractSourcePaths` / `combinedArmorSource`，并断言页面使用 `DetailArmorSetSkeleton`、`ArmorBuildMatrix`、`ArmorRecipeTable` 且总行数小于 800。合同源仍包含 `utils/armorEffectParsing.ts` 与 `composables/useArmorSetBuilds.ts`。
...
- [ ] **Step 1: 写失败合同**
  ...
  让用户模块合同读取共享 layout 源，断言两页均引用 `UserArticleEditorLayout`，同时继续逐页断言稳定 `<main>` 与各自 `form id`。内部字段、草稿恢复、封面裁剪、状态文案可在“页面 + layout”拼接源上检查。
...
- [ ] **Step 4: 收敛重复样式**
  ...
  将两个页面逐字相同的编辑器内部样式迁到共享 layout；只在 edit 页面保留加载骨架差异，只在页面保留 compact head 与 form shell 所需样式。不得改变断点、颜色、尺寸或文案。
