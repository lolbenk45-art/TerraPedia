...
| 1b | 迁移后复验 | 依赖任务 2 完成 |
| 3a | P3 临时回退 publicExposure → planned-public + publicRoute → null | 立即可执行，降低风险 |
...
- 仅禁用路由而保留 `publicExposure=public` 会使 A-grade gate 产生 `route-missing` warning（仅在 `--fail-on-warning=true` 下才阻断）；同时回退两者才能对齐旧计划"P0+P1 全部通过后才开放"的边界。注意 `planned-public` + `publicRoute=null` 仍会产生 `plannedRouteMissing` warning，这是预期行为（域已登记为计划公开但尚未上线）
...
**前置条件**：
- 任务 1b 迁移后复验全部 PASS
- `image-source-lineage-report.mjs --source=db` 显式暴露剩余 gap 与契约一致

注意：A-grade gate `--fail-on-warning=true` 不在前置条件中，因为在 3a 回退后 Items/NPCs 处于 `planned-public` + `publicRoute=null`，`plannedRouteMissing` warning 会导致 gate 失败。A-grade 强门禁应在选项 A 恢复公开配置后执行。

**两个选项**：

### 选项 A：迁移复验通过后恢复公开状态

1. 将 Items、NPCs 的 `publicExposure` 恢复为 `public`，`publicRoute` 恢复为路由配置
2. 运行 `domain-acceptance-a-grade-gate.mjs --fail-on-blocked=true --fail-on-warning=true`，确认无阻断/无 warning（此时 `planned-public` 已恢复为 `public`，`plannedRouteMissing` warning 应消失）
3. 前端公开路由页面功能验证
4. **与旧计划边界一致**：P0+P1 全部通过后才开放
