...
- [x] **Step 1:** `relationItems(items, detailBase)` 增加第二参数,生成 `href`:
...
- [x] **Step 2:** 模板(`buffs/[id].vue:199`)`<a ... href="/items">` 改为 `<NuxtLink :to="item.href">`(保留 class)。
- [x] **Step 3:** 人工验证:打开 `/buffs/1`,"来源"条目应跳对应物品详情;找一个有施加者的 debuff(如 `/buffs` 列表里的"中毒",来源计数 11)验证 NPC 跳转。
- [x] **Step 4:** `cd front-nuxt && pnpm run check`;commit `fix(buffs): route buff relation entries to their own detail pages`
...
- [x] **Step 1:** `items/index.vue:435` 改为 `const search = String(firstQueryValue(route.query.q ?? route.query.search) ?? '')`。
- [x] **Step 2:** 人工验证:访问 `/items?search=铁`,搜索框应带入"铁"且结果过滤生效。
- [x] **Step 3:** `pnpm run check`;commit `fix(items): accept legacy search query param from cross-page links`
...
- [x] **Step 1:** 逐页把卡片链接 `<a :href="x.detailPath">` 换成 `<NuxtLink :to="x.detailPath">`,class 与内部结构不动。注意:items 墙格若有 `@mouseenter` 聚焦逻辑保持在同一元素上。
- [x] **Step 2:** 人工验证:五个列表点卡片进详情应为 SPA 切换(无整页白屏),浏览器回退能恢复列表滚动位置。
- [x] **Step 3:** `pnpm run check`(其中 `check:public-pages`、`check:nav-layout` 会扫模板结构,若 contract 断言 `<a` 需同步更新对应 check 脚本断言);commit `fix(catalog): use NuxtLink for list-to-detail navigation`
...
- [x] **Step 1:** 给使用 dock 的列表容器加滚动余量,让最后一行能滚出 dock 覆盖区。推荐在 dock 自身规则旁新增兄弟规则(避免逐页改):
...
- [x] **Step 2:** 人工验证:四个列表页滚动到中部,dock 不应压住卡片文字;滚到底最后一行完整可见。
- [x] **Step 3:** `pnpm run check`;commit `fix(catalog): keep sticky pagination dock from covering list content`
...
- [x] **Step 1:** 卡片模板中 `{{ boss.bossType }}` 处改为 `{{ bossTypeLabel(String(boss.bossType ?? '').toLowerCase()) }}`;`bossTypeLabel` 的 fallback 保持 `'Boss'`。
- [x] **Step 2:** 人工验证 `/bosses`:卡片显示"困难模式前/困难模式/事件 Boss/小 Boss"。
- [x] **Step 3:** `pnpm run check`;commit `fix(bosses): render boss type with zh label instead of raw enum`
...
- [x] **Step 1:** 检查未找到块的 DOM:eyebrow 与 `<h1>`/`<b>` 是否在同一行盒;为标题元素加 `display: block`(或调整包裹结构),六个详情页统一。
- [x] **Step 2:** 人工验证 `/items/99999999`、`/bosses/99999`:标题独立成行、无粘连。
- [x] **Step 3:** `pnpm run check`(`check:detail-layout` 涉及详情布局 contract,若断言结构需同步);commit `fix(detail): unclamp not-found headline layout`
...
- [x] **Step 1(试点 items):** `usePublicItemDetail.ts` 去掉 `server: false`;确认 fetcher 在服务端走 `apiServerBase`、客户端走 `/api` 代理(对齐 `useHomeData` 的写法);确认 payload 序列化无 ref/函数。
- [x] **Step 2:** `items/[id].vue` 在 setup 顶层:数据解析后若确认未找到(fetch 完成且无 detail),`throw createError({ statusCode: 404, statusMessage: 'Item not found' })`;保留现有未找到卡片作为 `error.vue` 之外的兜底不再需要时移除。
- [x] **Step 3:** 验证试点:`curl -s http://localhost:<front>/items/1 | grep 铁镐` 应在 SSR HTML 里出现真实内容;`curl -o /dev/null -w '%{http_code}' http://localhost:<front>/items/99999999` 应为 404;view-source 的 `<title>` 应含物品名。**同时确认页面加载不再出现全屏骨架闪烁(visual loading 逻辑对 SSR 数据应短路)。**
- [x] **Step 4:** 复制到 bosses/npcs/biomes/buffs/armor-sets 五个详情;`pnpm run check && pnpm run test:unit`。
- [x] **Step 5:** 详情页补 `ogImage`(实体 imageUrl)与 canonical;commit 按实体拆分或一次 `feat(detail): server-render entity detail pages with real 404`
...
- [x] **Step 1:** `useVisualLoading(minimumMs)` 封装"最小时长骨架"三件套(timer/startedAt/sync),行为与现实现逐字一致(items 180ms、bosses 320ms 等以参数传入);先迁移 items + bosses 两页验证,再批量迁移其余 7 处。
- [x] **Step 2:** `useCatalogRouteSync` 封装 query 读写 + 防抖(300ms)+ **统一防回环 guard**(bosses 的 `syncingRouteQuery` 模式),迁移后 items/npcs 自动获得 guard,修复输入竞态(REVIEW §3.1-A);同步修复:bosses 双参数冗余(只写 `type`)、biomes 列表补 URL 同步。
- [x] **Step 3:** 错误通道:fetcher 改为返回 `{ data, error, source }` 结构(不再静默吞错),页面已写好的"载入异常/重试"分支(crafting/index.vue:346 等)接上真实 error;同步删除永不可达的 `fallbackCatalogItems` 假数据分支(usePublicItems.ts:131-150、items/index.vue:251-291、npcs 同款)。
- [x] **Step 4:** 每步 `pnpm run check`;分三个 commit(`refactor(catalog): extract useVisualLoading`、`refactor(catalog): extract useCatalogRouteSync`、`fix(catalog): surface fetch errors to retry branches`)
...
- [x] **Step 1:** ≤860px 显示汉堡按钮(`aria-expanded`/`aria-controls`),展开全屏抽屉:主链接 + 资料资源链接 + 账号快捷入口 + 主题切换,复用现有 resourceLinks/accountMenu 数据。
- [x] **Step 2:** 触屏交互:抽屉内取消 hover 逻辑,全部点击展开;Esc/遮罩点击关闭;打开时锁 body 滚动。
- [x] **Step 3:** 375px 截图回归(复用 `front-nuxt/tmp/visual-audit-capture.mjs` 脚本);`pnpm run check`(`check:nav-layout` contract 需同步扩展);commit `feat(nav): mobile drawer navigation`
...
- [x] **Step 1:** 编辑器:localStorage 草稿自动保存(内容变更防抖 3s,key 含文章 id/new);进入时检测未提交草稿提示恢复;`onBeforeRouteLeave` + `beforeunload` 有未保存变更时确认。
- [x] **Step 2:** 净化器:`sanitizeArticleHtml` 改为 DOMPurify 白名单配置(ALLOWED_TAGS 沿用现集合,`tp-content-ref`/`tp-recipe-tree` 的 data 属性经 hook 保留);**客户端渲染路径用 DOMPurify,SSR 路径(若 Task 8 之后文章页也 SSR)用 isomorphic-dompurify 或保留现正则实现作为服务端降级**——实施前确认文章详情是否走 SSR。
- [x] **Step 3:** 现有 `check:article-comments-runtime`、`check:user-article-editor*`、`check:article-content-references` 全绿;人工回归:引用 chip 悬浮预览、合成树嵌入、TOC 锚点。
- [x] **Step 4:** commit ×2(`feat(editor): draft autosave and leave guard`、`refactor(article): replace regex sanitizer with DOMPurify`)
...
P0+P1 全部完成于 `review/front-pages-audit`(P0: bd32f26/9285c61/12ad4de/910f115;P1: 355bb3e Task 8、becc71b+a38bb3d Task 10、c26069a Task 9、d646305+faacf89 Task 11)。`pnpm run check` 全绿;六详情页 curl 实测 SSR 内容/404/动态 title/ogImage;移动抽屉 Playwright 17 项行为断言通过。

与计划的偏差:
- Task 9 未删 usePublicItems.ts 的 fallbackCatalogItems 生成器(它是 fetcher 错误兜底的数据形状,页面侧不可达臂已删);三个 commit 合并为一个(改动在同批文件内交织)。items/npcs/buffs/armor-sets 四个 [id] 页为纯 computed 视觉态,不套 useVisualLoading(避免语义变更)。
- Task 10 抽屉需 ClientOnly+Teleport to body(site-nav 的 backdrop-filter 是 fixed 后代 containing block);Esc 监听挂 document;DOM 副作用收进 useMobileNavDrawer(public-pages contract 禁 TerraNav 出现 document.)。
- Task 11 DOMPurify 走 uponSanitizeElement hook 复用 sanitizeArticleAttributes;regex 白名单保留为 SSR+contract fixture 回退(未引入 isomorphic-dompurify)。
- 新发现存量问题(建议单独立项):a343bd4 在桌面端 display:none 掉实体列表页 page-head/h1(SEO/a11y 受损,check-visual-regression 持续红);h3 对未编码 UTF-8 query 返回 400(平台级)。
