...
- 状态：**待用户确认**（本文档由三个并行调研 agent 的现状调查 + 未获用户逐条确认的默认选型草拟而成，见「未确认项」一节）
...
## 6. 未确认项（用户离开期间按最匹配需求原文的默认值推进，需回来后确认）

以下三点在设计过程中因用户未及时响应而采用了推荐默认值，**并非最终定论**，请在评审本文档时明确是否调整：

1. **事件范围**：目前只做「文章审核」+「爬虫监控」两类（用户原话明确提到的两类）。评论审核等其他类型未纳入第一期。
2. **数据来源**：选择了纯前端轮询 + diff（不改后端），换来的代价是已读状态/历史不跨设备、不跨浏览器同步。如果后续需要跨设备一致的通知状态，需要新增后端聚合接口，这会是设计的重大调整而非增量修改。
3. **展示形式**：选择了顶部铃铛 + 下拉面板（而非右侧抽屉）。
