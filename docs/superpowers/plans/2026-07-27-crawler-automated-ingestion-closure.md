...
The runner must reject empty fields, expired packets, hash drift, database
fingerprint drift, operation mismatch, reused decision identity, or an approval
issued for another bundle.
...
request freezes the available ordered bundle bytes, database fingerprints,
policy hash, operation ID, and expiry, reports the still-missing Owner fields,
and carries `authorizationStatus: "AWAITING_OWNER"`. Request generation may read
formal schema/fingerprints but must not mutate databases.
...
`canonical-npc-crawler`, `canonical-npc-apply`,
`automation-biomes-first-l1`, `automation-biomes-l2-promotion`, and
`automation-biomes-scheduler-activation`. Their request files are respectively
`canonical-item-group-bootstrap.request.json`,
`canonical-npc-crawler.request.json`, `canonical-npc-apply.request.json`,
`automation-biomes-first-l1.request.json`,
`automation-biomes-l2-promotion.request.json`, and
`automation-biomes-scheduler-activation.request.json` under
`reports/authorization/canonical/`.
...
- [ ] **Step 1: Request the first L1 decision for the fixed `biomes` candidate**
...
- [ ] **Step 4: Freeze and approve the exact `biomes` preview bundle**
- [ ] **Step 5: Execute `biomes` L1 apply under its independent approval**
