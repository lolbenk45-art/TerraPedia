# Standalone Crawler Monitor Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a dedicated read-only crawler monitor service on its own port that mirrors the existing admin crawler monitor page style and refreshes live crawler state without depending on the admin app, login, Nuxt proxy, or backend API.

**Architecture:** Create a small Node.js standalone HTTP service under `scripts/dev/` that reads only local JSON files and directories, exposes `/api/state` plus a single HTML page, and serves no mutation routes. The page copies the visual language of `data-query-app/pages/operations/crawler-monitor.vue` and `data-query-app/assets/css/main.css`: hero header, summary cards, status cards, progress rows, file health, recent reports, and restrained admin dashboard styling.

**Tech Stack:** Node.js ESM HTTP server, filesystem reads, vanilla HTML/CSS/JS, Node test runner.
...
- New read-only service at `http://127.0.0.1:3099`.
- Page visually modeled after the existing admin crawler monitor page.
- Realtime polling every 2 seconds.
- Direct file reads for crawler state.
- Tests proving read-only behavior, payload shape, and style anchors.

Out of scope:

- Starting, stopping, retrying, or scheduling crawlers.
- Backend API dependency.
- Login/auth dependency.
- DB reads or writes.
- Replacing the existing admin monitor page.

## Source Paths

The standalone monitor reads:

- `data/generated/wiki-sync-progress.latest.json`
- `/home/lolben/data/terraPedia/generated/wiki-request-gate.latest.json`
- `/home/lolben/data/terraPedia/generated/fetch-wiki-buffs-progress.latest.json`
- `/home/lolben/data/terraPedia/raw/wiki/item-pages`
- `/home/lolben/data/terraPedia/reports/fetch`
- `reports/crawler-monitor`

Configurable environment variables:

- `TERRAPEDIA_MONITOR_PORT`, default `3099`
- `TERRAPEDIA_MONITOR_HOST`, default `127.0.0.1`
- `TERRAPEDIA_SHARED_DATA_ROOT`, default resolved by existing project-root helpers when possible

## Files

- Create: `scripts/dev/standalone-crawler-monitor.mjs`
- Create: `scripts/dev/standalone-crawler-monitor.test.mjs`
- Modify: `docs/superpowers/plans/2026-06-03-standalone-crawler-monitor.md`

Do not modify:

- `data-query-app/pages/operations/crawler-monitor.vue`
- `data-query-app/assets/css/main.css`
- backend monitor controllers/services

## Design Details

### Page Layout

The standalone page must copy the admin monitor visual pattern:

- Outer `page-wrap crawler-monitor`
- `workspace-shell workspace-shell--unified`
- Hero with `CRAWLER MONITOR`, title `爬取监控`, subtitle, and summary mini cards
- `status-grid` cards
- `source-progress-panel` for item page progress
- `operations-grid` with active task, request gate, raw outputs, reports
- `monitor-layout` with main progress rows and side file health
...
- Links that trigger crawler actions

The only user actions:

- Manual refresh
- Auto refresh interval toggle or display
- Report path display
...
```json
{
  ...
  "repoRoot": "/home/lolben/TerraPedia",
  "sharedDataRoot": "/home/lolben/data/terraPedia",
  "itemPages": {
    "rawCount": 70,
    "rawDir": "/home/lolben/data/terraPedia/raw/wiki/item-pages",
    "latestReport": {
      "path": "/home/lolben/data/terraPedia/reports/fetch/fetch-item-pages-2026-06-03T03-45-37.667Z.json",
      "successCount": 50,
      "failureCount": 0,
      "withRecipes": false
    }
  ...
  "files": [
    {
      "label": "Item page progress",
      "path": "data/generated/wiki-sync-progress.latest.json",
      "found": true,
      "readable": true,
      "updatedAt": "2026-06-03T03:52:32.480Z"
    }
  ],
  "recentReports": []
}
...
- [ ] **Step 1: Add tests for state collection and read-only routes**
...
```bash
node --test scripts/dev/standalone-crawler-monitor.test.mjs
```

Expected: fail because `scripts/dev/standalone-crawler-monitor.mjs` does not exist.

## Task 2: Implement Standalone Monitor Service
...
Create `scripts/dev/standalone-crawler-monitor.mjs` with:
...
## Task 3: Add Runtime Smoke

**Files:**
- Modify: `scripts/dev/standalone-crawler-monitor.test.mjs`
...
## Task 4: Start Dedicated Monitor Port
...
```bash
TERRAPEDIA_MONITOR_PORT=3099 node scripts/dev/standalone-crawler-monitor.mjs
```

Expected:

```text
Standalone crawler monitor: http://127.0.0.1:3099
```
...
```bash
curl -s http://127.0.0.1:3099/api/state | node -e "let s='';process.stdin.on('data',d=>s+=d);process.stdin.on('end',()=>{const o=JSON.parse(s); console.log(JSON.stringify({raw:o.itemPages.rawCount,status:o.progress.payload?.status,cooldown:o.requestGate.cooldownActive}, null, 2));})"
```
...
Expected: page matches the backend crawler monitor layout style, refreshes every 2 seconds, and shows no mutation controls.
...
```bash
node --test scripts/dev/standalone-crawler-monitor.test.mjs
curl -s http://127.0.0.1:3099/api/state | head
```

If the service is not running, the curl command should fail; start it with Task 4.

## Plan Self-Review

- Spec coverage: covers independent port, copied monitor page style, realtime polling, read-only routes, local file state, and no backend dependency.
- Placeholder scan: no placeholders.
- Type consistency: `buildStandaloneMonitorState`, `renderStandaloneMonitorHtml`, and `routeStandaloneMonitorRequest` are defined before test usage.
- Residual risk: vanilla CSS copies the admin monitor visual language rather than importing Nuxt CSS directly. This is intentional so the monitor can run without Nuxt.
