...
- [ ] **Step 3: Implement the collector with only authenticated read operations.** Query the production monitor's automation read endpoint and its V2 status/reconciler view; obtain the durable epoch/namespace and lease/attempt membership from existing read-only monitor/repository APIs, not raw JSON mutation or Redis writes. Read each eligible-domain readiness record and require its authoritative readiness predicate to pass. Hash every source record, record server/endpoint identity without tokens, and reject missing, inconsistent, or older-than-window observations.
  ...
  **Code-bundle set must be manifest-derived, not operator-chosen (review-added, boundary defect B).** The current CLI accepts an arbitrary `--code-bundle=<path>` file list, so the frozen bundle is only as complete as what the operator typed. This defeats the "freeze the exact code bundle" guarantee: an operator can pass a partial or wrong set and the preflight still hashes and blesses it. The collector must instead derive the canonical code-bundle path set for `canonical-crawler-v2-scheduler-activation` from `buildCanonicalOperationExecutionManifest(...).codeBundleEntries` (which transitively expands static imports and fails closed on a missing file) and reject any `--code-bundle` input whose path set is not exactly equal to that manifest-declared set. The preflight then hashes the manifest-declared files. Add a negative test: a bundle that omits or adds a path, or reorders it, must fail closed.
...
- [ ] **Step 3: Replace the CLI's representative `current` constants with a validated preflight input.** The CLI must require explicit `--preflight`, `--t1-report`, and `--output` paths, hash each before construction, reject paths outside the repository authorization root, emit `authorizationStatus:"AWAITING_OWNER"`, and never invoke the authorization packet builder or permit writer. Preserve these forbidden operations in the proposal: `direct-json-write`, `direct-redis-write`, `manual-sweep`, `external-daemon`, and `formal-permit-consumption`.
  ...
  **Bundle-set drift must fail closed in the builder too (review-added, boundary defect B/C).** Two gaps exist in the committed builder: (1) it never cross-checks the preflight's `codeBundle` path set against the manifest-declared set, so a preflight built with a partial bundle still yields a valid proposal; and (2) `assertCodeBundle` uses `codeBundle ?? expected`, so calling `buildCrawlerV2SchedulerActivationProposal({ t1Report, preflight })` with no explicit `codeBundle` makes the drift check compare `expected` to itself and always pass — a no-op defense-in-depth layer. Fix both: the proposal CLI must recompute the canonical bundle path set from `buildCanonicalOperationExecutionManifest('canonical-crawler-v2-scheduler-activation').codeBundleEntries` and reject a preflight whose bundle path set differs; and the builder must treat a missing `codeBundle` argument as an error rather than defaulting to `preflight.codeBundle`. Add negative tests for both.
...
- [ ] **Step 2: Re-observe production state immediately before enabling (review-added, boundary defect A — TOCTOU).** The preflight freshness window is `MAX_PREFLIGHT_AGE_MS = 15 minutes`, but the owner-approval gap between the Task 5 request and this enablement is unbounded (hours or days). A packet-vs-request hash match (Step 1) proves the *decision* is internally consistent; it does not prove the *production state* is still safe. Before issuing any enable body, run the read-only preflight GET one more time and abort unless the freshly observed state still matches the permit-bound preflight on every safety axis: same epoch and namespace, `enabled:false` / `mode:'changed-only'` config, zero live attempts, zero sweep claims, healthy reconciler with zero overdue/failed work, and every proposed domain still eligible with an unchanged source hash. Any drift invalidates the permit — do not enable; open the Task 5 loop again with a new preflight and request.

- [ ] **Step 3: Enable automation through the authenticated loopback API only.** Issue the manifest-bound `PUT /admin/crawler-monitor/v2/automation` enable body, then observe a real scheduled tick without calling a manual sweep or starting an external daemon. Record attempt ID, domain, source mode, progress, lease ownership/renewal, reconciler health, and database/relationship outcomes through the controlled API/read-only readers.

- [ ] **Step 4: Enforce the production observation budget.** Abort on an unapproved domain, source/network-policy violation, missing progress identity, stale heartbeat, lease conflict, failed reconciler, unexpected DB target, or output-count mismatch. Immediately use the same authenticated API to restore `{ enabled:false, mode:'changed-only' }`; do not compensate by direct Redis/JSON/SQL writes.

- [ ] **Step 5: Verify rollback and postconditions independently.** Require disabled changed-only configuration, zero live attempts, zero sweep claims, no retained lease/permit, healthy reconciler, no owned child/daemon, and an immutable result showing the authorized observation outcome. If the scheduler had no approved successful tick, treat this as a failed-closed activation and open an incident; do not retry with the consumed identity.
...
### Review findings (2026-08-10 handoff review)

- **A — TOCTOU at enablement (plan defect, now fixed in Task 6 Step 2).** The committed plan re-ran the preflight only at request time (Task 5 Step 1). With a 15-minute preflight freshness window and an unbounded owner-approval gap before Task 6, automation could have been enabled against drifted production state (a manually started crawler, a rotated epoch, a degraded reconciler) that no fresh observation ever re-checked. Task 6 now requires a fresh read-only preflight immediately before the enable `PUT`, aborting on any safety-axis drift.
- **B — code-bundle set was operator-chosen, not manifest-bound (plan + code gap, IMPLEMENTED).** Both the preflight collector CLI (`--code-bundle=<path>`) and the proposal builder accepted whatever bundle the operator supplied and never cross-checked it against the operation's manifest-declared `CODE_PATHS`. Hashing an incomplete set still produced a "valid" frozen bundle, defeating the freeze guarantee. Fixed: the preflight now **derives** its bundle from `buildCanonicalOperationExecutionManifest('canonical-crawler-v2-scheduler-activation').codeBundleEntries` (19 transitively-expanded files) via `resolveSchedulerActivationCodeBundlePaths`, and no longer accepts `--code-bundle`; the proposal CLI cross-checks the preflight's bundle path set against that manifest set and fails closed on any drift.
- **C — builder `codeBundle` fallback was a no-op (code gap, IMPLEMENTED).** `assertCodeBundle(codeBundle ?? expected, ...)` compared the preflight bundle to itself when no explicit `codeBundle` argument was passed, so the programmatic drift check silently passed. Fixed: the builder now requires an explicit non-empty `codeBundle` and throws otherwise instead of defaulting to `preflight.codeBundle`.
- Scope note: A stays gated (it is Task 6 enablement logic and requires fresh owner/ADMIN authorization). B and C were preparation-boundary hardening (proposal-only, no production mutation) and are implemented under TDD in this review: negative tests for a missing bundle and a manifest-set-drifted bundle, plus a manifest-derivation test. Focused lanes: scheduler preflight + proposal `14/14`; with manifest + cutover `80/80`; whole automation `315/315`.
...
- Risk: a valid proposal can become stale before authorization or enablement. The 15-minute preflight window covers request time but not the unbounded approval gap before enablement; Task 6 Step 2 closes this with a fresh pre-enable preflight (finding A).
...
- Closed corrective work: findings B and C are now implemented under TDD. The preflight derives its code bundle from the operation manifest (19 files) and the proposal CLI rejects any preflight bundle whose path set drifts from that manifest; the builder requires an explicit code bundle. Finding A remains gated to Task 6 and unimplemented by design pending fresh owner authorization.
