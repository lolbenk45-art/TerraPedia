...
因此，`M3` 不再只是“有页面”，而是已具备“可持续维护、可批量复核、可解释保存结果”的最小闭环。
...
### 1. 尚未做本轮运行态 API smoke

本轮已完成测试与构建验证，但没有启动本地三端，对真实 DB 运行：

- `GET /admin/town-npcs/maintenance`
- `PUT /admin/npcs/{id}`

原因：

- 当前收口重点是代码层契约与构建层验收。
- 真实 DB 运行态 smoke 会写入或依赖当前本地库状态，应作为切换 `M4` 前最后一步执行。

建议：

- 切 `M4` 前跑一次本地栈 smoke。
- 如果 smoke 通过，`M3` 可正式标记完成。

### 2. 未匹配售卖物仍是数据治理工作，不是代码阻塞

历史 import report 中仍存在未匹配售卖物，例如晶塔、节日物品、特殊服装等。

这些问题当前已经能在维护台被看见和复核，不再属于“链路不可解释”，但仍属于后续数据治理待办。

### 3. Public NPC 消费面不是 M3 范围
...
- **代码层 / 构建层：可以准备切换 M4。**
- **严格验收层：建议先跑一次本地运行态 smoke，再切 M4。**

建议下一步：

1. 启动本地栈。
2. 对 `GET /admin/town-npcs/maintenance` 做一次真实响应抽验。
3. 选一个测试 NPC 执行一次保存，确认 `shopMutationSummary` 返回。
4. 若通过，则把 `M3` 标记为完成，下一任务切入 `M4：配方与关系域闭环`。
